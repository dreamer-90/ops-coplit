...
from app.schemas import ScramRequest, DispatchRequest, EngineDecision, AuditLogRecord, EmergencyState
...
async def trigger_event_business_logic(index: int) -> dict:
    ...
    decision = await engine_process_event(event, MockDecisionHistory(decisions), current_state)
...
async def activate_scram_business_logic(req: ScramRequest) -> dict:
    if req.level >= 3:
        ...
        if req.override_code != expected_code:
            raise HTTPException(status_code=403, detail="Invalid manager override code for SCRAM Level 3+")
